class_name MySprite extends RefCounted

var texture: Texture2D = null
var sprite_dimensions = Vector2.ZERO
...
func _init(t: Texture2D, rows: int, cols: int):
	texture = t
	rows_cols.x = rows
	rows_cols.y = cols
...
func draw_sprite(node2d: Node2D, texture_manager, pos: Vector2, scale: float, clip, sprite_coords = Vector2.ZERO):
	var dest_dim = scale * Settings.WIDTH / 2 * (2000 * texture_manager.sprite_scale) * sprite_dimensions
